...
public final class Agent {
	...
	public static void premain(String args, Instrumentation inst) {
		...
		System.out.println("+---------------------------------------------------+");
		System.out.println("| Jalen Agent Version 2                             |");
		System.out.println("+---------------------------------------------------+");
		...
		if (!OSValidator.isUnix()) {
			...
			Agent.LOGGER.log(Level.SEVERE, "Only Linux-based systems aare supported");
		} else {
			// Linux-based systems
			...
			Agent.LOGGER.log(Level.INFO, "CPU...OK");
		...
		Runtime.getRuntime().addShutdownHook(new Thread() {
			@Override
			public void run() {
				Agent.LOGGER.log(Level.INFO, "Calculating and writing energy data for PID " + appPid + " in folder " + Agent.userDir);
				...
				Agent.appendToFile(Agent.userDir + "/" + appPid + ".csv", netCon);
				if (!Agent.filterMethodName.equals(""))
					Agent.appendToFile(Agent.userDir + "/" + appPid + "-filtered.csv", netLibraryCon);
			}
		});
	}
	 ...
	 */
	public static void appendToFile(String fileName, String methData) {
		try {
